...
pub fn get_git_token() -> Option<String> {
    ...
    if global_config_path.exists() {
        let config = Config::load_from_file(global_config_path.to_str().expect("Invalid path"))?;
        let config = Config::load_from_file(&global_config_path).expect("Could not read global config file");
